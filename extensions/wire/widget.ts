...
import { showPeerSession } from "./session-view";
        ...
        if (this.wire.poolSelected !== null) {
            ...
            if (!this.isShowingAutocomplete()) {
                if (this.kb.matches(data, "tui.select.confirm")) {
                    const rows = poolRows(this.wire);
                    const peer = rows[Math.max(0, Math.min(this.wire.poolSelected, rows.length - 1))];
                    // Never let selecting a peer submit an unfinished prompt.
                    this.wire.poolSelected = null;
                    this.repaintPool();
                    if (peer) void showPeerSession(this.wire, peer);
                    return;
                }
...
export function poolRows(state: WireState, registryEntries: RegistryEntry[] = peekCachedEntries()): Row[] {
    ...
    const endpoints = new Map(registryEntries.map((entry) => [entry.session_id, entry.endpoint]));
    ...
    for (const [sid, card] of state.peerCards.entries()) {
        ...
        rows.push({
            name: card.name,
            endpoint: endpoints.get(sid),
            session_file: card.session_file,
            model: card.model,
            color: card.color,
...
export function renderPool(
    ...
    if (selected !== null) {
        bottomBorder = ruleWithHint(theme, ` ${selected + 1} of ${rows.length} · ↑↓ move · enter view · esc back `, safeWidth);
